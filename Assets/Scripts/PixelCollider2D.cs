...
// mod from RandomiaGaming's https://github.com/RandomiaGaming/Unity-Pixel-Perfect-Collider-2D
public sealed class PixelCollider2D : MonoBehaviour
{
    [Range(0, 1)]
    public float alphaCutoff = 0.5f;
    public bool Sprite_on_child;
    public List<List<Vector2>> Regenerate(bool sprite_on_child)
    {
        ...
        Pixel_Paths = Get_Unit_Paths(SR.sprite.texture, alphaCutoff, SR.sprite.rect); //sends rect over to *only* get collider from current slice sprite
        ...
        World_Paths = Finalize_Paths(Pixel_Paths, SR.sprite, SR.sprite.rect);
    ...
    private List<List<Vector2>> Finalize_Paths(List<List<Vector2Int>> Pixel_Paths, Sprite sprite, Rect section)
    {
        Vector2 pivot = sprite.pivot;
        pivot.x *= Mathf.Abs(sprite.bounds.max.x - sprite.bounds.min.x);
        pivot.x /= section.width;
        pivot.y *= Mathf.Abs(sprite.bounds.max.y - sprite.bounds.min.y);
        pivot.y /= section.height;
    ...
    private static List<List<Vector2Int>> Get_Unit_Paths(Texture2D texture, float alphaCutoff, Rect section)
    {
        List<List<Vector2Int>> Output = new List<List<Vector2Int>>();
        for (int x = (int)section.x; x < section.width; x++)
        {
            for (int y = (int)section.y; y < section.height; y++)
            {
...
public class PixelColider2DEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        {
            PC2D.Regenerate(PC2D.Sprite_on_child);
        }
    }
}
#endif