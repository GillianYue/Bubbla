...
public class BGMover : MonoBehaviour {
	...
	void Start(){
        ...
        {
            Global.resizeSpriteToRectX(background, background.GetComponent<SpriteRenderer>().sprite, transform.parent.gameObject); //parent is legit Canvas
        ...
        bool first = true;
            foreach (RectTransform rt in scrollTransforms) {
                    ...
                    Global.resizeSpriteToRectX(background, scr.sprite, transform.parent.gameObject); //scaling GO to parent canvas (will then affect child size)
                    Global.zeroX(background);

                    if (first)
                    {
                        //setting limits to bg sprite scroll
                        bottomY = -rt.localScale.y * scr.sprite.rect.y;
                        topY = -bottomY;
                    }
